...
\end{center}
\caption{Schematic of twisted, multi-core fiber consisting of $N$ waveguides arranged in a ring.}
\label{fig:ring}
\end{figure}
...
\end{equation}
where $a_n \in \R$, $\theta_n \in (-\pi/2, \pi/2]$, and $\omega$ is the frequency of oscillation. (Since we allow $a_n$ to be negative, we can restrict $\theta_n$ to that interval). \revised{Standing waves are periodic in $z$ with period $2\pi/\omega$, and the intensity at each node $|c_n| = |a_n|$ is constant in $z$}.
